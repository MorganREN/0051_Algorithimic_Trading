...
class MovingAverage:
    def __init__(self, df, V_0, L, short_window, long_window) -> None:
        '''
        ...
        L : Leverage
        *_window : Window size for moving average
        '''
        self.signals = None
        self.df = df
        self.V_0 = V_0
    ...
    def generate_signals(self):
        ...
        self.signals['Signal'][0] = 1
        self.signals['Short_MA'] = self.df['Adj Close'].rolling(window=self.short_window, min_periods=1, center=False).mean()

        self.signals['Long_MA'] = self.df['Adj Close'].rolling(window=self.long_window, min_periods=1, center=False).mean()

        self.signals['Signal'][self.short_window:] = np.where(self.signals['Short_MA'][self.short_window:] > self.signals['Long_MA'][self.short_window:], 1.0, -1.0)

        self.signals['Position'] = self.signals['Signal'].diff()
        self.signals['Position'] = self.signals['Position'].replace(-2.0, -1.0)
        self.signals['Position'] = self.signals['Position'].replace(2.0, 1.0)
    ...
    def execute_strategy(self):
        ...
        for i in range(1, len(self.df)):
            theta_t = self.stock_hold[i-1] * self.df['Adj Close'][i] * self.signals['Signal'][i]
            # if |theta| > theta_0
            ...
            # if theta_0 - V_0 < |theta| <= theta_0
                ...
                cap_V_t = 0
                hold_t = abs(self.stock_hold[-1]) * self.signals['Signal'][i]
                PnL_t = self.df['Return/Unit'][i] * theta_t * self.signals['Signal'][i]
    ...
    def get_signals(self):
        ...
        '''
        return np.array(self.signals['Position'])
    ...
    def get_returns(self):
        ...
        Get the returns
        '''
        return np.array(self.df['Return/Unit'])
