...
class MeanReversion:
    def __init__(self, df, V_0, L, window):
        '''
        ...
        L : Leverage
        window : Window for the moving average
        '''
        self.signals = None
        self.df = df
        self.V_0 = V_0
    ...
    def generate_signals(self):
        ...
        self.signals['MA'] = self.df['Adj Close'].rolling(window=self.window, min_periods=1, center=False).mean()

        self.signals['Signal'][self.window:] = np.where(self.df['Adj Close'][self.window:] < self.signals['MA'][self.window:], 1.0, -1.0)
